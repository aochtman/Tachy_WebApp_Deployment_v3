                    ...
                    [
                        html.Div(
                            [
                                ...
                                html.Div(
                                    [
                                        html.H6("Prediction Normal", className="well"), 
                                        html.P("Loading...", className="test", id="text_gt"),
                                        dbc.Progress("",id='progress_gt',value=0,striped=True,animated=True,color="info",style={"height": "20px", "margin":"-8px 4px 0px 4px", 'font-size':'14px'})
                                    ],id="gt",
                                    className="col mini_container",
                                ),
                                html.Div(
                                    [
                                        html.H6("Prediction Split", className="well"), 
                                ...
                                html.Div(
                                    [
                                        html.H6("Prediction Unclassified", className="well"), 
                                        html.P(".", className="test", id="text_error"),
                                        dbc.Progress(".",value=0,striped=True,animated=True,color="danger",style={"height": "20px", "margin":"-8px 4px 0px 4px", 'font-size':'14px'})
                                    ],id="error",
                                    className="col mini_container",
                                ),
                                html.Div(
                                    [
                                        html.H6("No Predictions", className="well"), 
                                        html.P(".", className="test", id="text_error23"),
                                        dbc.Progress(".",value=0,striped=True,animated=True,color="progress",style={"height": "20px", "margin":"-8px 4px 0px 4px", 'font-size':'14px'})
                                    ],id="error234",
                                    className="col mini_container",
                                ),
                                # html.Div(
                                #     [html.H6("Coming soon...",id="waterText", className="well"), html.P(" ", className="test")],
                                #     id="water",
                                #     className="col mini_container",
                                # ),
                            ],
                ...
                html.Div(
                    [   
                        ...
                        html.Div(
                            [
                                html.Div(
                                    [
                                        ...
                                        # -- Datatable --
                                        html.H6("Data Table", className="well",style={'margin-bottom': '1rem'}),
                                        dash_table.DataTable(
                                            id='datatable',
                                            style_cell_conditional=[
                                                {'if': {'column_id': 'id'},
                            ...
                            ], className='row col-md-3 offset-md-9 fixed-bottom', style={'max_height':'90%'},
                        )
                    ],
                    className="col-md-3",
                ),
                ...
                html.Div(
                    [
                        dcc.Tabs(id='tabs-example', value='tab-1', children=[
                            ...
                            #         animate=False)
                            #     ]),
                            dcc.Tab(label='Model Timings, Voltage & EGM Graph', value='tab-1', className="myTab", selected_className="myTab1", children=[
                                html.Div(
                                    [
                                        html.Div([
                            ...
                            dcc.Tab(label='Model Classes', value='tab-2', className="myTab", selected_className="myTab2", children=[dcc.Graph(id="graph_classes2",style={'height': '650px', 'width':'100%'})]),
                        ]),
                        html.Div(id='tabs-example-content')
                    ],
                    className="col-md-9 pretty_container",
                ),
            ],
            className="row flex-display",
        ),
...
def reload_data(value,active_cell,selected_cells,start_cell,end_cell):
    ...
    layout = dict(
        ...
        legend_title='<b> Timings Model </b>',
        legend = dict(
            x=0,
            y=900,
            bordercolor="white",
    ...
    TARGET = dict(
        ...
        text = target['id'],
        hoverinfo = 'text',
        marker = dict( size=20, color="white", opacity=1, line=dict(width=2,color='DarkSlateGrey'))
    )
    target_5nn = dict(
        mode = "markers",
        name = "Target's 5NN",
        ...
        text = target_5nn['id'],
        hoverinfo = 'text',
        marker = dict( size=12, color="pink", opacity=1, line=dict(width=2,color='DarkSlateGrey'))
    )
    ...
    allpoints_timings = dict(
        ...
        text = df['id'].values,
        hoverinfo = 'text',
        marker = dict( size=8, color=df['t'].values, colorscale='Jet', colorbar=dict(title="Time Scale"), line=dict(width=2,color='DarkSlateGrey'))
    )
    ...
    fig_predictions = dict( data=[TARGET,target_5nn,normal,split,uncl,rest], layout=layout)
    ...
    fig_ground_truth = dict( data=[TARGET,target_5nn,normal_gt,split_gt,uncl_gt,rest_gt], layout=layout)
    ...
    return fig_timings, fig_voltage, fig_predictions, fig_ground_truth, text_gt, perc_gt, text_gt_progress, text_pred, perc_pred, text_pred_progress, text_target, graph_egm

############
# Workflow 4 - Update Data
############
